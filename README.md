# RemindMe — Event Reminder App (Lite)

Многопользовательский планировщик событий с уведомлениями в **Telegram** и на **Email** и веб-интерфейсом.

> **Ветка `lite`** — облегчённая версия без Prometheus, Grafana и Loki. Инфраструктура: только PostgreSQL + приложение.
...
Spring Boot автоматически поднимет PostgreSQL через Docker Compose.
...
Поднимает два контейнера: `postgres` и `app`.
...
```bash
# Запустить приложение (авто-поднимает PostgreSQL)
